...
void Terrain::Initialize(D3D*d3d, char* setupFilename)
{
...
void Terrain::LoadSetupFile(char* filename)
{
	...
	m_terrainFilename = make_unique<char[]>(stringLength);
...
void Terrain::LoadRawHeightMap()
{
	int32_t error, i, j, index;
	FILE* filePtr;
	...
	m_heightMap = make_unique<HeightMapType[]>(m_terrainWidth * m_terrainHeight);

	// Open the 16 bit raw height map file for reading in binary.
	error = fopen_s(&filePtr, m_terrainFilename.get(), "rb");
	ThrowIfTrue(error != 0);
	...
	ThrowIfTrue(!rawImage);

	// Read in the raw image data.
	count = fread(rawImage, sizeof(unsigned short), imageSize, filePtr);
	ThrowIfTrue(count != imageSize);
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			...
			// Store the height at this point32_t in the height map array.
...
{
	int32_t i, j, index1, index2, index3, index;
...
void Terrain::LoadColorMap()
{
	int32_t error, imageSize, i, j, k, index;
	FILE* filePtr;
	...
	error = fopen_s(&filePtr, m_colorMapFilename.get(), "rb");
	ThrowIfTrue(error != 0);
	...
	ThrowIfTrue((bitmapInfoHeader.biWidth != m_terrainWidth) || (bitmapInfoHeader.biHeight != m_terrainHeight));
...
{
	int32_t i, j, index, index1, index2, index3, index4;
	...
	m_terrainModel = make_unique<ModelType[]>(m_vertexCount);
...
void Terrain::LoadTerrainCells(D3D* d3d)
{
	auto device = d3d->GetDevice();
	int32_t cellHeight, cellWidth, cellRowCount, i, j, index;
	...
	m_TerrainCells = make_unique<TerrainCell[]>(m_cellCount);
	ThrowIfTrue(!m_TerrainCells);
	...
	for(j=0; j<cellRowCount; j++)
	{
		for(i=0; i<cellRowCount; i++)
		{
			...
			m_TerrainCells[index].Initialize(d3d, m_terrainModel.get(), i, j, cellHeight, cellWidth, m_terrainWidth);
		}
	}
}

bool Terrain::RenderCell(ID3D12GraphicsCommandList* commandList, int32_t cellId, Frustum* Frustum)
{
	...
	m_TerrainCells[cellId].Render(commandList);
...
void Terrain::RenderCellLines(ID3D12GraphicsCommandList* commandList, int32_t cellId)
{
	m_TerrainCells[cellId].RenderLineBuffers(commandList);
	return;
}


int32_t Terrain::GetCellIndexCount(int32_t cellId)
{
	return m_TerrainCells[cellId].GetIndexCount();
}


int32_t Terrain::GetCellLinesIndexCount(int32_t cellId)
{
	return m_TerrainCells[cellId].GetLineBuffersIndexCount();
}


int32_t Terrain::GetCellCount()
{
	return m_cellCount;
}


int32_t Terrain::GetRenderCount()
{
	return m_renderCount;
}


int32_t Terrain::GetCellsDrawn()
{
	return m_cellsDrawn;
}


int32_t Terrain::GetCellsCulled()
{
