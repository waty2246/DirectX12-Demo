...
bool Frustum::CheckPoint(float x, float y, float z)
{
	...
	// Check each of the six planes to make sure the point32_t is inside all of them and hence inside the frustum.
	for(i=0; i<6; i++)
	{
		...
		// Determine if the point32_t is on the correct side of the current plane, exit out if it is not.
		if(dotProduct <= 0.0f)
		{
			return false;
		}
