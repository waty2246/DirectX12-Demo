...
void FontShader::Initialize(D3D* d3d)
{
	m_d3d = d3d;
	auto device = d3d->GetDevice();

	// Initialize the vertex and pixel shaders.
	InitializeShader(device, GetShaderFilePath("font_vs.hlsl"), GetShaderFilePath("font_ps.hlsl"));
}

void FontShader::Render(ID3D12GraphicsCommandList* commandList, uint32_t indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D12Resource* texture, XMFLOAT4 pixelColor)
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters(commandList, worldMatrix, viewMatrix, projectionMatrix, texture, pixelColor);
...
void FontShader::InitializeShader(ID3D12Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	ThrowIfFailed(D3DCompileFromFile(vsFilename, NULL, NULL, "FontVertexShader", "vs_5_0", compileFlags, 0, &vertexShader, nullptr));

	// Compile the pixel shader code.
	ThrowIfFailed(D3DCompileFromFile(psFilename, NULL, NULL, "FontPixelShader", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
	...
	LOG_DEBUG("[Create Font Shader Pipeline State]");
...
void FontShader::SetShaderParameters(ID3D12GraphicsCommandList* commandList, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D12Resource* texture, XMFLOAT4 pixelColor)
{
	...
	ThrowIfFailed(m_pixelConstantBuffer->Map(0, &readRange, reinterpret_cast<void**>(&pixelData)));
	pixelData->pixelColor = pixelColor;
	m_pixelConstantBuffer->Unmap(0, nullptr);

	commandList->SetGraphicsRootSignature(m_rootSignature.Get());

	commandList->SetGraphicsRootConstantBufferView(0, m_matrixConstantBuffer->GetGPUVirtualAddress());
	commandList->SetGraphicsRootConstantBufferView(1, m_pixelConstantBuffer->GetGPUVirtualAddress());
...
void FontShader::RenderShader(ID3D12GraphicsCommandList* commandList, uint32_t indexCount)
{
	PIXBeginEvent(commandList, 0, L"Draw Using FontShader");
